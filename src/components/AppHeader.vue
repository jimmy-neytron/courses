...
const initials = computed(() => String(auth.user?.user_metadata?.display_name ?? auth.user?.email ?? 'U')
  .split(/[ @]/)
  .filter(Boolean)
  .slice(0, 2)
...
<template>
  <header class="app-header">
    <PrimeButton class="menu" severity="secondary" text rounded aria-label="Открыть меню" @click="layout.openSidebar"><Menu /></PrimeButton>
