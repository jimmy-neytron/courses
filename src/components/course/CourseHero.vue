...
<template>
  <section class="product-course-hero">
    ...
    <div class="product-course-copy">
      ...
      <div class="product-course-meta"><span><BookOpen />{{ moduleCount }} {{ text.modules }}</span><span><FileText />{{ lessonCount }} {{ text.lessons }}</span><span><Clock3 />{{ formatDuration(totalMinutes) }} программы</span></div>
    </div>
