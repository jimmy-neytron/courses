...
<template>
  <section class="block-fieldset">
    <header><div><strong>Карточки</strong><small>{{ block.cards?.length ?? 0 }} шт.</small></div><PrimeButton size="small" severity="secondary" outlined @click="addCard"><Plus />Добавить</PrimeButton></header>
    <div class="repeatable-list">
      <article v-for="(card, index) in block.cards" :key="index" class="repeatable-card">
        ...
        <InputText v-model="card.front" placeholder="Слово или вопрос" @update:model-value="emit('change')" />
        <InputText v-model="card.back" placeholder="Перевод или ответ" @update:model-value="emit('change')" />
        <InputText v-model="card.hint" placeholder="Подсказка для запоминания" @update:model-value="emit('change')" />
      </article>
    </div>
