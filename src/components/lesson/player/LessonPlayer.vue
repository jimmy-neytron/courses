...
import LessonBlockContent from '@/components/lesson/player/LessonBlockContent.vue'
import LessonQuizQuestion from '@/components/lesson/player/LessonQuizQuestion.vue'
...
<template>
  <div class="engine-player">
    <header class="engine-lesson-header">
      <div>
        <p class="eyebrow">
          {{ courseTitle }} · {{ lesson.duration }} минут
        </p>
      ...
      <span
        ...
        class="engine-guest-note"
      >
        Гостевой просмотр · прогресс не сохраняется
      </span>
    ...
    >
      ...
      >
        <LessonQuizQuestion
          ...
          :result="answerResults[block.id]"
          :error="answerErrors[block.id]"
          :pending="checkingAnswers[block.id]"
          :theory-available="sections.some(
            (section) => section.id === 'theory',
          )"
        ...
        <LessonBlockContent
          v-else
          :block="block"
          @complete="markSection(currentSection.id)"
        />
      </template>

      <div
        v-if="currentQuestions.length && answeredCount"
        class="engine-test-result"
      >
        <span>Результат</span>
        ...
        <p v-else>
          {{
            correctCount === questions.length
              ? 'Все ответы верны'
              : 'Изучите объяснения и попробуйте ещё раз'
          }}
        </p>
      </div>
      ...
      >
        ...
        {{
          completedSections.includes(currentSection.id)
            ? 'Раздел изучен'
            : 'Я изучил этот раздел'
        }}
      </button>

      <span
        v-else
        class="engine-guest-footer"
      >
        Войдите, чтобы сохранять прогресс разделов
      </span>
    </main>

    <section
      v-else
      class="product-empty compact"
    >
      <BookOpen />
      <h3>В уроке пока нет доступных разделов</h3>
      <p>
        Автор может включить разделы и добавить материалы
        в редакторе.
      </p>
    </section>
