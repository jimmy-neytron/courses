...
export function useLessonEditor() {
  ...
  async function addBlock(type: BlockType, afterIndex = selectedIndex.value, sectionId?: LessonSectionId) {
    ...
    try {
      ...
      if (sectionId) {
        ...
        await store.saveBlock(found.value.lesson.id, addedBlock.id)
      }
    ...
    } catch (error) { setError(error, 'Не удалось добавить блок') }
  }

  async function chooseBlock(type: BlockType) {
    blockPickerOpen.value = false
    await addBlock(type, insertAfterIndex.value, insertSectionId.value || undefined)
  }
  ...
  const persistChanges = useDebounceFn(async () => {
    ...
    try {
      ...
      showSaved()
    } catch (error) { setError(error, 'Не удалось сохранить изменения') } finally { saving.value = false }
  }, 700)
  ...
  async function assignBlockSection(block: LessonBlock, sectionId: LessonSectionId) {
    ...
    selectedId.value = block.id
    saving.value = true
    try { await store.saveBlock(found.value.lesson.id, block.id); showSaved() } catch (error) { block.sectionId = previousSectionId; setError(error, 'Не удалось изменить раздел блока') } finally { saving.value = false }
  }
  ...
  async function saveSections() {
    if (!found.value) return
    const nextSections = sectionDraft.value.map((section, order) => ({ ...section, label: section.label.trim(), order })).filter((section) => section.label)
    if (!nextSections.length) { editorError.value = 'Нужен хотя бы один раздел'; return }
    const removedIds = new Set(availableSections.value.filter((section) => !nextSections.some((item) => item.id === section.id)).map((section) => section.id))
    const fallbackId = nextSections[0]!.id
    const movedBlocks = blocks.value.filter((block) => removedIds.has(resolveLessonBlockSection(block, availableSections.value, courseKind.value)))
    movedBlocks.forEach((block) => { block.sectionId = fallbackId })
    sectionSaving.value = true
    try {
      await Promise.all(movedBlocks.map((block) => store.saveBlock(found.value!.lesson.id, block.id)))
