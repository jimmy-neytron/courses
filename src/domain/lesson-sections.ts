...
export function getDefaultLessonSections(
  kind: CourseKind,
): ReadonlyArray<LessonSectionConfig> {
  return kind === 'language'
    ? languageLessonSections
    : generalLessonSections
}
...
export function createLessonSectionConfig(
  ...
  const sections = source?.length
    ...
    : getDefaultLessonSections(kind).map(
        (section) => ({ ...section }),
      )
...
export function resolveLessonBlockSection(
  ...
  const available = createLessonSectionConfig(
    sections,
    kind,
  )
  ...
  const blockTypes = kind === 'language'
    ? languageSectionBlockTypes
    : generalSectionBlockTypes

  return available.find(
    (section) => blockTypes[section.id]?.includes(block.type),
  )?.id
    ?? available[0]?.id
    ?? 'content'
}
