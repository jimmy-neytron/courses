...
<template>
  <FullscreenLayout>
    <div v-if="course" class="engine-course-shell">
      ...
      <main class="engine-course-main"><header class="engine-course-topbar"><AppBreadcrumbs :items="[{ label: 'Курсы', to: '/app/courses' }, { label: course.title, to: `/app/courses/${course.id}` }, { label: selected?.title ?? 'Просмотр' }]" /><RouterLink :to="`/app/courses/${course.id}`" class="icon-button" aria-label="Закрыть курс"><X /></RouterLink></header><LessonPlayer v-if="selected" :lesson="selected" /><section v-else class="product-empty full"><BookOpen /><h2>В курсе пока нет уроков</h2></section></main>
    </div>
